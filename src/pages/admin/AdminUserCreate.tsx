/**
 * AdminUserCreate - Página completa para crear un nuevo usuario
 */

import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '@clerk/clerk-react';
import { createUser, getAllTenants, getAllRoles, getAllPaises, TenantAdmin, Role, Pais, CreateUserData } from '../../services/api';
...
export default function AdminUserCreate() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const [tenantsData, rolesData, paisesData] = await Promise.all([
        getAllTenants(token),
        getAllRoles(token),
        getAllPaises(token),
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userType === 'tenant' && (!formData.tenantIds || formData.tenantIds.length === 0)) {
      ...
      formData.tenantIds?.forEach(tenantId => {
  ...
  const handleTenantToggle = (tenantId: string) => {
    ...
    if (currentTenants.includes(tenantId)) {
      setFormData({
        ...formData,
        tenantIds: currentTenants.filter(id => id !== tenantId),
      });
      // Limpiar los roles seleccionados para este tenant
      const newSelectedRoles = { ...selectedTenantRoles };
      delete newSelectedRoles[tenantId];
      setSelectedTenantRoles(newSelectedRoles);
    } else {
      setFormData({
        ...formData,
        tenantIds: [...currentTenants, tenantId],
      });
    }
  };

  const handlePlatformRoleToggle = (roleId: string) => {
    if (selectedPlatformRoles.includes(roleId)) {
      setSelectedPlatformRoles(selectedPlatformRoles.filter(id => id !== roleId));
    } else {
      setSelectedPlatformRoles([...selectedPlatformRoles, roleId]);
    }
  };

  const handleTenantRoleToggle = (tenantId: string, roleId: string) => {
    const currentRoles = selectedTenantRoles[tenantId] || [];
    if (currentRoles.includes(roleId)) {
      setSelectedTenantRoles({
        ...selectedTenantRoles,
        [tenantId]: currentRoles.filter(id => id !== roleId),
      });
    } else {
      setSelectedTenantRoles({
        ...selectedTenantRoles,
        [tenantId]: [...currentRoles, roleId],
      });
    }
  };

  const tenantRoles = roles.filter(r => r.tipo === 'tenant');
  const platformRoles = roles.filter(r => r.tipo === 'platform');

  if (loading) {
    return (
      <div className="admin-user-create-loading">
      ...
      <div className="create-container">
        ...
        <form onSubmit={handleSubmit} className="create-form">
                ...
                <label className={`user-type-option ${userType === 'platform' ? 'active' : ''}`}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setFormData({ ...formData, tenantIds: [] });
                      setSelectedTenantRoles({});
                    }}
                  />
                    ...
                    {platformRoles.map((role) => (
                      <label key={role.id} className="role-checkbox-label">
                        <input
                          ...
                          onChange={() => handlePlatformRoleToggle(role.id)}
                        />
              ...
              <div className="form-group" style={{ marginTop: '32px' }}>
                ...
                {tenants.length === 0 ? (
                  ...
                  <div className="tenants-list">
                    {tenants.map((tenant) => {
                      const isSelected = formData.tenantIds?.includes(tenant.id) || false;
                                ...
                                {tenantRoles.map((role) => (
                                  <label key={role.id} className="role-checkbox-label">
                                    <input
                                      ...
                                      onChange={() => handleTenantRoleToggle(tenant.id, role.id)}
                                    />
                                    <div className="role-info">
                                      <span className="role-name">{role.nombre}</span>
                                      {role.descripcion && (
          ...
          <div className="form-actions">
            ...
            <button
              type="submit"
              disabled={saving || !formData.email.trim() || !formData.password.trim() || formData.password !== confirmPassword || !formData.nombre.trim() || !formData.apellido.trim() || !userType || (userType === 'platform' && selectedPlatformRoles.length === 0) || (userType === 'tenant' && (!formData.tenantIds || formData.tenantIds.length === 0)) || (userType === 'both' && selectedPlatformRoles.length === 0 && (!formData.tenantIds || formData.tenantIds.length === 0))}
